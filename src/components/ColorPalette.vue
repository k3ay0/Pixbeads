<template>
  <div v-if="!colors || colors.length === 0" class="text-xs text-center text-gray-500 dark:text-gray-400 py-2">
    当前图纸无可用颜色。
  </div>
  <div v-else class="bg-white dark:bg-gray-900 rounded border border-blue-200 dark:border-gray-700">
    <!-- 色板切换按钮区域 -->
    <div
      v-if="fullPaletteColors && fullPaletteColors.length > 0 && onToggleFullPalette"
      class="flex justify-center p-2 border-b border-blue-100 dark:border-gray-700"
    >
      <button
    ...
    >
      <div class="text-center">
        ...
        <div v-if="colorReplaceState.step === 'select-source'" class="text-xs text-purple-600 dark:text-purple-400">
          ...
          <p class="text-gray-500 dark:text-gray-400">选择后将高亮显示该颜色的所有位置</p>
        </div>
        <div v-else class="text-xs text-purple-600 dark:text-purple-400">
          <p class="mb-1">步骤 2/2：从下方色板选择替换成的颜色</p>
          <div class="flex items-center justify-center gap-2 mt-2">
            <span class="text-gray-500 dark:text-gray-400">被替换的颜色：</span>
            <div class="flex items-center gap-1">
              <span
                class="inline-block w-4 h-4 rounded border border-gray-400 dark:border-gray-500"
    ...
    >
      <div class="text-center">
        ...
        <div class="text-xs text-orange-600 dark:text-orange-400">
          ...
          <p class="text-gray-500 dark:text-gray-400">使用洪水填充算法，一次性擦除连通的相同颜色区域</p>
        </div>
      </div>
    </div>
    ...
    <div
      ...
      class="p-3 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50"
    >
      <div class="text-center">
        <div class="flex items-center justify-center gap-2 mb-2">
          ...
          <span class="text-sm font-medium text-gray-700 dark:text-gray-300">橡皮擦模式</span>
        </div>
        <div class="text-xs text-gray-600 dark:text-gray-400">
          <p class="mb-1">点击图中任意位置清除单个格子</p>
          <p class="text-gray-500 dark:text-gray-400">逐个删除不需要的颜色，不会影响其他格子</p>
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-wrap justify-center gap-2 p-2">
      ...
      <button
        v-if="onEraseToggle"
        @click="onEraseToggle"
        class="w-12 h-12 rounded border-2 flex-shrink-0 transition-transform transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-400 dark:focus:ring-blue-500 flex items-center justify-center"
        :class="isEraseMode
      ...
      >
        <svg
          ...
          class="h-5 w-5"
          :class="isEraseMode ? 'text-red-600 dark:text-red-400' : 'text-orange-600 dark:text-orange-400'"
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          :stroke-width="2"
      ...
      <button
        ...
        class="relative w-12 h-12 rounded border-2 flex-shrink-0 transition-transform transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-400 dark:focus:ring-blue-500 flex items-center justify-center"
        :class="[
          isTransparent(colorData) ? 'bg-gray-100 dark:bg-gray-700' : '',

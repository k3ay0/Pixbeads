...
// 分类选择状态：none 未选 / partial 部分 / all 全选
function categoryState(cat: { prefix: string; total: number; selected: number }): 'none' | 'partial' | 'all' {
  if (cat.selected === 0) return 'none'
  if (cat.selected === cat.total) return 'all'
  return 'partial'
}

// 切换某分类下所有颜色的选择状态
function toggleCategory(cat: { prefix: string; total: number; selected: number }) {
  const select = cat.selected !== cat.total
    ; (props.allColors as any[]).forEach((color: any) => {
      const matches = cat.prefix === '全部' || getColorPrefix(color.hex) === cat.prefix
      if (matches) {
        tempSelections.value[color.hex.toUpperCase()] = select
      }
    })
}
...
<template>
  <Teleport to="body">
    <div class="modal-overlay backdrop-blur-sm" @click.self="handleClose">
        ...
        <div class="flex-1 overflow-y-auto">
          <div class="p-4 sm:p-6">
            <div class="flex flex-col h-full max-h-[calc(90vh-40px)]">
              ...
              <div class="flex flex-col gap-2 pb-3 border-b border-gray-200 dark:border-gray-700">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                  <div class="flex h-8 rounded-lg bg-gray-100 dark:bg-gray-800 overflow-hidden flex-shrink-0">
                    <button v-for="system in colorSystemOptions" :key="system.key"
                      class="px-3 text-[11px] font-medium transition-colors" :class="selectedColorSystem === system.key
                  ...
                  <div class="relative flex-1 min-w-[120px]">
                    <input v-model="searchTerm" placeholder="搜索色号"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg"
                      class="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-400" fill="none"
              ...
              <div class="flex-1 flex min-h-0 mt-0 sm:mt-3 gap-3">
                ...
                <div class="hidden sm:flex w-28 flex-shrink-0 flex-col gap-1 overflow-y-auto overscroll-contain pr-1">
                  <button v-for="cat in categories" :key="cat.prefix"
                    ...
                    <span
                      class="w-3.5 h-3.5 flex-shrink-0 rounded border flex items-center justify-center transition-colors"
                      :class="categoryState(cat) === 'all'
                        ? 'bg-brand-500 border-brand-500 text-white'
                        : categoryState(cat) === 'partial'
                          ? 'border-brand-500 text-brand-500'
                          : 'border-gray-300 dark:border-gray-600'" @click.stop="toggleCategory(cat)">
                      <svg v-if="categoryState(cat) === 'all'" class="w-2.5 h-2.5" viewBox="0 0 12 12" fill="none"
                        stroke="currentColor" stroke-width="2.5">
                        <path d="M2 6l3 3 5-5" />
                      </svg>
                      <svg v-else-if="categoryState(cat) === 'partial'" class="w-2.5 h-2.5" viewBox="0 0 12 12"
                        fill="none" stroke="currentColor" stroke-width="2.5">
                        <path d="M3 6h6" />
                      </svg>
                    </span>
                    <span class="flex flex-col min-w-0 flex-1">
                      <span class="truncate leading-tight">{{ cat.prefix }}</span>
                      <span class="tabular-nums text-[10px] leading-tight text-gray-400 dark:text-gray-500">
                ...
                <!-- 移动端分类横滑 -->
                <div
                  class="sm:hidden flex gap-1.5 overflow-x-auto [&::-webkit-scrollbar]:hidden [scrollbar-width:none] py-2 flex-shrink-0">
                  <button v-for="cat in categories" :key="cat.prefix"
                    class="flex-shrink-0 px-3 py-1.5 text-xs rounded-lg transition-colors whitespace-nowrap" :class="activeCategory === cat.prefix
                      ? 'bg-gray-900 text-gray-50 dark:bg-gray-200 dark:text-gray-900'
                      : 'text-gray-600 dark:text-gray-400 active:bg-gray-200 dark:active:bg-gray-700'"
                    @click="activeCategory = cat.prefix">
                    {{ cat.prefix }}
                    <span class="tabular-nums"
                      :class="activeCategory === cat.prefix ? '' : 'text-gray-500 dark:text-gray-400'">
                      {{ cat.selected }}/{{ cat.total }}
                    </span>
                  </button>
                </div>
                ...
                <div class="flex-1 overflow-y-auto overflow-x-hidden overscroll-contain p-1">
                  <div class="flex flex-wrap gap-1.5 content-start">
                    <button v-for="color in displayedColors" :key="color.hex"
              ...
              <!-- 底部操作栏 -->
              <div
                class="flex items-center justify-between flex-wrap gap-2 pt-3 mt-3 border-t border-gray-200 dark:border-gray-700">
                <div class="flex items-center gap-2 flex-wrap">
                  <button

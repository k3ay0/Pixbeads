  ...
  <div class="flex flex-col h-full max-h-[calc(90vh-80px)]">
    ...
    <div class="flex justify-between items-center border-b dark:border-gray-700 pb-3 mb-3">
      <h2 class="text-lg font-semibold text-gray-800 dark:text-gray-100 flex items-center">
        ...
        色板管理中心 <span class="ml-2 text-sm text-blue-500 dark:text-blue-400">({{ selectedCount }} 色)</span>
      </h2>
      <button
        @click="emit('close')"
    ...
    <div class="mb-4">
      <div class="relative">
        <input
          ...
          placeholder="搜索色号..."
          v-model="searchTerm"
          class="w-full px-3 py-2 pl-9 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 focus:ring-blue-500 focus:border-blue-500"
        />
    ...
    <!-- 说明文本 -->
    <div class="mb-4 text-xs text-gray-600 dark:text-gray-400 bg-blue-50 dark:bg-blue-900/20 p-2 rounded-md border border-blue-100 dark:border-blue-800/30">
      <p class="flex items-start">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1 text-blue-500 flex-shrink-0 mt-0.5" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div class="flex-1 overflow-y-auto pr-1">
      <div
        ...
        :key="prefix"
        class="mb-3 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden"
      >
        <!-- 组标题 -->
        <div
          class="flex justify-between items-center px-3 py-2 bg-gray-50 dark:bg-gray-800 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-750"
          @click="toggleGroup(prefix)"
        >
          <div class="flex items-center">
            <span class="font-medium text-gray-800 dark:text-gray-200">{{ prefix }} 系列</span>
          ...
          <div class="flex items-center">
            ...
            <button
              ...
              class="text-xs text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300 mr-2"
            >
              全不选
            </button>
            ...
            <svg
              ...
              :class="['h-4 w-4 text-gray-500 dark:text-gray-400 transform transition-transform', expandedGroups[prefix] ? 'rotate-180' : '']"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
        ...
        <div v-if="expandedGroups[prefix]" class="p-3 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
          <label
            ...
            :key="color.key"
            class="flex items-center space-x-2 p-1.5 hover:bg-gray-50 dark:hover:bg-gray-750 rounded cursor-pointer"
          >
            <input
              type="checkbox"
            ...
            />
            <div
              class="w-6 h-6 rounded-sm border border-gray-300 dark:border-gray-600 flex-shrink-0"
              :style="{ backgroundColor: color.hex }"
            />
            <span class="text-sm text-gray-800 dark:text-gray-200">{{ getColorKeyByHex(color.hex, selectedColorSystem) }}</span>
          </label>
        </div>
      </div>
    </div>

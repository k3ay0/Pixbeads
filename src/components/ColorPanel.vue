    ...
    <div class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-end" @click.self="$emit('close')">
      <div class="w-full bg-white rounded-t-2xl max-h-[80vh] flex flex-col">
          ...
          >
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center space-x-2">
                <!-- 完成勾选 -->
                <div v-if="Math.round((colorInfo.completed / colorInfo.total) * 100) === 100" class="text-green-500">
                  <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
                  </svg>
                </div>
                <!-- 选中勾选 -->
                <div v-if="colorInfo.color === currentColor" class="text-blue-500">
                  <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div class="mt-2 w-full bg-gray-200 rounded-full h-1.5">
              <div
                ...
                :class="Math.round((colorInfo.completed / colorInfo.total) * 100) === 100 ? 'bg-green-500' : 'bg-blue-500'"
        ...
        <div class="p-4 border-t border-gray-200">
          <button
            @click="$emit('close')"
            class="w-full py-3 bg-gray-500 text-white rounded-lg hover:bg-gray-600 transition-colors"
          >
            关闭
          </button>
        </div>

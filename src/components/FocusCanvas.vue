...
// 坐标标签字体大小（超边框时自动缩小，避免三位数重叠）
const coordinateFontSize = computed(() => {
  if (!props.showCoordinates) return 12
  const cs = cellSize.value
  let fs = Math.max(9, Math.min(12, Math.floor(cs * 0.7)))
  // 受格子宽度约束：相邻列号文字宽度不超过列间距
  if (props.gridDimensions) {
    const maxNum = Math.max(props.gridDimensions.N, props.gridDimensions.M)
    const digits = maxNum.toString().length
    const colStep = props.coordinateInterval <= 1 ? cs : cs * props.coordinateInterval
    const maxFontByCol = Math.floor(colStep / (digits * 0.6))
    if (maxFontByCol > 0) fs = Math.min(fs, maxFontByCol)
  }
  return Math.max(6, fs) // 保底可读性
})
...
function renderCanvas() {
  ...
  for (let row = 0; row < M; row++) {
    for (let col = 0; col < N; col++) {
      ...
      if (!pixel) continue
      // 空格子（透明/外部格子）跳过绘制，避免白色被映射成 T01 色号
...
<template>
  <div ref="containerRef" class="w-full h-full flex items-center justify-center overflow-hidden bg-black/[0.04]"
    style="touch-action: none">
    <canvas ref="canvasRef" class="cursor-crosshair border border-black/10"
      style="image-rendering: pixelated; image-rendering: crisp-edges;" @click="handleClick" @wheel="handleWheel"
